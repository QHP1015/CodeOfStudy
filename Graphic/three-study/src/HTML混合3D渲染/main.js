...
function createDom(type, className, content, postionX, postionY, postionZ) {
  const ele = document.createElement(type);
  ele.className = className;
  ele.innerHTML = content;
  const label = new CSS2DObject(ele);
  label.position.set(postionX, postionY, postionZ);
  return label;
}
...
function init() {
  ...
  const earthLabel = createDom("div", "label", "地球", 0, 1, 0);
  ...
  chinaLabel = createDom("div", "label1", "中国", -0.3, 0.5, -0.9);
  ...
  const moonLabel = createDom("div", "label", "月球", 0, 0.3, 0);
  moon.add(moonLabel);
